...
执行流程：`getLine` 读取输入 → `tokenize` 拆分为独立单元 → `parser` 构建 AST → `executeAST` 分派到单命令、管道或顺序执行。

## 当前限制

- `&&`、`||` 已在语法结构中识别并按顺序执行，但尚未实现基于退出状态的短路逻辑。
- 后台运行符 `&` 已在词法分析中识别，但尚未实现对应的后台执行逻辑。
- 尚未支持环境变量展开、通配符展开。
